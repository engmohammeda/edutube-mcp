...
import {
  ...
  edutubeStartInputSchema,
  runLessonExtraction,
  type EdutubeStartInput,
...
} from "@engmohammeda/edutube-core";
import { getLongVideoRuntimeCapabilities, type LongVideoRuntimeCapabilities } from "@engmohammeda/edutube-core/youtube/runtime";
...
export function createServer(options: CreateServerOptions = {}): McpServer {
  ...
  server.registerTool(
    ...
    {
      title: "Start Edutube Lesson Extraction (playlist or video)",
      description: [
        "Paste a YouTube PLAYLIST URL (or playlist id, or a single video URL) and this job extracts a structured lesson per video:",
        "dialogue with Arabic translations, key expressions, vocabulary, Arabic notes, and a 10-question quiz, written as lesson-NN.json + lessons-all.json + summary.json into outputDir.",
        "Uses the long-video path first and falls back to 150s windowed extraction. Skips already-valid lesson files (resume-safe).",
        "Automatic Gemini model rotation applies on quota errors. Poll with get_edutube_job_status, read with get_edutube_job_result, cancel with cancel_edutube_job.",
      ].join(" "),
      inputSchema: edutubeStartInputSchema,
      outputSchema: startLongAnalysisJobOutputSchema,
    },
    async (args) => {
      const input = args as EdutubeStartInput;
      const logger = createRequestLogger("start_edutube_extraction");
      const startedAt = Date.now();
      logger.info("tool.start", { url: input.url, maxVideos: input.maxVideos ?? null, outputDir: input.outputDir ?? null });

      try {
        const job = longAnalysisJobStore.createJob({ youtubeUrl: input.url } as LongToolInput, DEFAULT_TASK_TTL_MS);
        const output: StartLongAnalysisJobOutput = {
          jobId: job.jobId,
          status: "queued",
          statusMessage: "Edutube extraction job queued.",
        };
        ...
        void Promise.resolve().then(async () => {
          ...
          try {
            ...
            const result = await runLessonExtraction(
              ai,
              new InMemoryAnalysisSessionStore(),
              {
                url: input.url,
            ...
            if (currentJob.controller.signal.aborted) {
              longAnalysisJobStore.cancelJob(job.jobId, "Edutube extraction job cancelled.");
              return;
            }

            longAnalysisJobStore.completeJob(job.jobId, result as unknown as LongToolOutput);
            logger.info("job.success", {
              durationMs: Date.now() - startedAt,
              jobId: job.jobId,
              source: result.source,
            ...
            const jobError = toJobError(error, "start_edutube_extraction");
            longAnalysisJobStore.failJob(job.jobId, jobError);
            logger.error("job.failure", { durationMs: Date.now() - startedAt, jobId: job.jobId, ...jobError });
          }
        });
        ...
        const diagnostic = asDiagnosticError(error, {
          tool: "start_edutube_extraction",
          code: "EDUTUBE_JOB_START_FAILED",
          stage: "unknown",
          message: "Failed to start edutube extraction job.",
        });
        logger.error("tool.failure", { code: diagnostic.code, stage: diagnostic.stage, message: diagnostic.message });
        return createErrorToolResult("start_edutube_extraction", logger.requestId, diagnostic);
      }
    }
  );

  server.registerTool(
    "get_edutube_job_status",
    {
      title: "Get Edutube Job Status",
      description: "Returns the status and latest progress for a job created by start_edutube_extraction.",
      inputSchema: longAnalysisJobIdInputSchema,
      outputSchema: longAnalysisJobStatusOutputSchema,
    },
    async ({ jobId }) => {
      const logger = createRequestLogger("get_edutube_job_status");
      logger.info("tool.start", { jobId });
      const job = longAnalysisJobStore.getJob(jobId);
      if (!job) {
        return createErrorToolResult("get_edutube_job_status", logger.requestId, createJobNotFoundError("get_edutube_job_status", jobId));
      }
      logger.info("tool.success", { jobId, status: job.status });
      return createSuccessToolResult(createJobStatusOutput(job));
    }
  ...
  server.registerTool(
    "get_edutube_job_result",
    {
      title: "Get Edutube Job Result",
      description: "Returns the final summary (entries, totals, outputDir) for a done job created by start_edutube_extraction.",
      // No outputSchema: the edutube result payload is a custom run summary.
      inputSchema: longAnalysisJobIdInputSchema,
    },
    async ({ jobId }) => {
      const logger = createRequestLogger("get_edutube_job_result");
      logger.info("tool.start", { jobId });
      const job = longAnalysisJobStore.getJob(jobId);
      if (!job) {
        return createErrorToolResult("get_edutube_job_result", logger.requestId, createJobNotFoundError("get_edutube_job_result", jobId));
      }
      logger.info("tool.success", { jobId, status: job.status });
      return createSuccessToolResult(createJobResultOutput(job));
    }
  ...
  server.registerTool(
    "cancel_edutube_job",
    {
      title: "Cancel Edutube Job",
      description: "Cancels a queued or running job created by start_edutube_extraction.",
      inputSchema: longAnalysisJobIdInputSchema,
      outputSchema: cancelLongAnalysisJobOutputSchema,
    },
    async ({ jobId }) => {
      const logger = createRequestLogger("cancel_edutube_job");
      logger.info("tool.start", { jobId });
      const job = longAnalysisJobStore.cancelJob(jobId);
      if (!job) {
        return createErrorToolResult("cancel_edutube_job", logger.requestId, createJobNotFoundError("cancel_edutube_job", jobId));
      }
      logger.info("tool.success", { jobId, status: job.status });
      return createSuccessToolResult(createCancelJobOutput(job));
    }
