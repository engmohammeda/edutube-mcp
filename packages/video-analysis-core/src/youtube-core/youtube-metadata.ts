...
// LOCAL PATCH (agent-env fix): long-video metadata originally hard-required
// the YouTube Data API (YOUTUBE_API_KEY), even though the README lists that
// key as optional. When the key is missing, fetch the same fields locally
// with yt-dlp --dump-json (no download) instead.
async function fetchLongVideoMetadataViaYtDlp(options: FetchYouTubeVideoMetadataOptions): Promise<YtDlpMetadata> {
  const command = process.env.YT_DLP_PATH?.trim() || "yt-dlp";
  let stdout: string;
  try {
    const result = await execFileAsync(
      command,
      ["--no-warnings", "--no-playlist", "--skip-download", "--dump-json", options.youtubeUrl],
      {
        timeout: options.timeoutMs ?? METADATA_TIMEOUT_MS,
    ...
    throw new DiagnosticError({
      ...
      code: "YOUTUBE_METADATA_FETCH_FAILED",
      stage: "metadata",
      message: "yt-dlp metadata fallback failed. Set YOUTUBE_API_KEY or make yt-dlp available.",
      retryable: false,
      cause: error,
    });
  }
  ...
  const durationSeconds = typeof info.duration === "number" ? info.duration : Number(info.duration);
  const uploadDateRaw = typeof info.upload_date === "string" && /^\d{8}$/.test(info.upload_date)
  ...
  return {
    ...
    liveStatus: info.is_live === true ? "live" : "none",
  };
}
...
export async function fetchLongVideoMetadata(options: FetchYouTubeVideoMetadataOptions): Promise<YtDlpMetadata> {
  ...
  if (hasYoutubeApiKey) {
    const apiMetadata = await fetchYouTubeVideoMetadata(options);
    metadata = {
      durationSeconds: apiMetadata.durationSeconds ?? 0,
      title: apiMetadata.title,
      uploader: apiMetadata.channelTitle,
      uploadDate: apiMetadata.publishedAt,
      liveStatus: apiMetadata.liveBroadcastContent,
    };
  } else {
    metadata = await fetchLongVideoMetadataViaYtDlp(options);
  }
  ...
  if (!metadata.durationSeconds || metadata.durationSeconds <= 0) {
    throw new DiagnosticError({
      ...
      stage: "metadata",
      message: "Video metadata did not include a usable video duration.",
      retryable: false,
      details: {
