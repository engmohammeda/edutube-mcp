// LOCAL PATCH (agent-env feature): automatic cross-model rotation.
// Every Gemini model has its own free-tier quota pool. When one model returns
// RESOURCE_EXHAUSTED (429), the rotator transparently switches to the next
// available model so long batches can keep running. The model catalogue is
// discovered live from the Gemini API, so future models (e.g. gemini-3.6-flash,
// gemini-3.7-*) join the rotation without code changes.
...
async function discoverModels(apiKey: string): Promise<string[]> {
  if (!apiKey) return [];
  if (discoveredCache && Date.now() - discoveredCache.at < CACHE_TTL_MS) return discoveredCache.models;
  try {
    ...
    discoveredCache = { at: Date.now(), models: names };
    return names;
  } catch {
...
export class ModelRotator {
  ...
  async pick(): Promise<string | null> {
    ...
    const discovered = await discoverModels(this.apiKey);
    ...
    for (const c of candidates) {
      if (this.tried.has(c) || schemaUnsupported.has(c)) continue;
      const until = exhaustedUntil.get(c);
  ...
  markTried(model: string): void {
    this.tried.add(model);
  }
  ...
  markQuotaExhausted(model: string, retrySeconds: number | null): void {
    const secs = Math.min(Math.max(retrySeconds ?? 3600, 300), 6 * 3600);
    exhaustedUntil.set(model, Date.now() + secs * 1000);
  }
