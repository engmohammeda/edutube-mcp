...
// LOCAL PATCH (agent-env fix): YouTube now serves separate video/audio
// streams for most videos, so the original merged-only selector ("b...")
// failed with "Requested format is not available". Allow separate streams
// that yt-dlp merges into a single file (see --merge-output-format below).
// Keep mp4/m4a combinations first so the merge stays Gemini-friendly.
export const YT_DLP_DEFAULT_FORMAT =
  "b[height<=480][ext=mp4]/bv*[height<=480][ext=mp4]+ba[ext=m4a]/bv*[height<=480]+ba[ext=m4a]/bv*[height<=480]+ba/b[height<=480]/b";
...
export const USER_CONFIG_DIR_NAME = "edutube-mcp";
