...
async function generateStructuredJsonOnce(
  ai: GoogleGenAI,
  params: {
    model: string;
    prompt: string;
...
// LOCAL PATCH (agent-env feature): transparent model rotation across Gemini
// free-tier quota pools. On 429/quota errors the same request is retried on the
// next available model; cached content is dropped when switching models because
// Gemini caches are model-specific.
